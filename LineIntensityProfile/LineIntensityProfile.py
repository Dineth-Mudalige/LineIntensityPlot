...
class LineIntensityProfileTest(ScriptedLoadableModuleTest):
  ...
  def test_LineIntensityProfile1(self):
    ...
    import urllib
    downloads1 = (
        ('http://www.slicer.org/w/img_auth.php/5/59/RegLib_C01_1.nrrd', 'RegLib_C01_1.nrrd', slicer.util.loadVolume),
        )

    for url,name,loader in downloads1:
    ...
    volumeNode1 = slicer.util.getNode(pattern="RegLib_C01_1")
    logic = LineIntensityProfileLogic()
    self.assertTrue( logic.hasImageData(volumeNode1) )

    downloads2 = (
        ('http://www.slicer.org/w/img_auth.php/e/e3/RegLib_C01_2.nrrd', 'RegLib_C01_2.nrrd', slicer.util.loadVolume),
        )

    for url,name,loader in downloads2:
      filePath = slicer.app.temporaryPath + '//' + name
      if not os.path.exists(filePath) or os.stat(filePath).st_size == 0:
        print('Requesting download %s from %s...\n' % (name, url))
        urllib.request.urlretrieve(url, filePath)
    ...
    volumeNode2 = slicer.util.getNode(pattern="RegLib_C01_2")
    logic = LineIntensityProfileLogic()
